...
approx_agg_init(AggState *aggstate)
{
	...
	aggstate->top_k = palloc0(sizeof(ApproxTopEntry) * agg->approx_nkeep);
        for (i=0;i<agg->approx_nkeep;i++)			
		(aggstate->top_k+i)->count =0;
...
approx_agg_per_input(AggState *aggstate, TupleTableSlot* outerSlot, Agg* agg)
{
	 ...
	 */
/*
	uint32 estimated_value=0;
        uint32 *hash_values = palloc0(agg->cm_depth*sizeof(uint32));
	uint32_t min=0, min_index=0;	
	bool found = false;
	//int i=0;
	ApproxTopEntry* temp;
	//ApproxTopEntry* entry=palloc0(sizeof(ApproxTopEntry));

	getTupleHashBits(aggstate, outerSlot, hash_values, agg->cm_width, agg->cm_depth); //computes hash value

	increment_bits(aggstate->cm, hash_values);
	estimated_value = estimate(aggstate->cm, hash_values);  //get estimated value from Count-Min sketch
	
	approx_agg_reset_iter(aggstate);

	temp = approx_agg_advance_iter(aggstate, agg);

	while(temp){		//update if the tuple is already in the list
		if (compare_tuple_with_approx_top_tuple(outerSlot, temp, aggstate, agg)){
			temp->count = estimated_value;
			found = true;
			break;
		}
		temp = approx_agg_advance_iter(aggstate, agg);
	...
	if (!found){
         	...
         	if (aggstate->array_walker < agg->approx_nkeep){       // if array has less than k items
			set_approx_top_entry_from_slot(outerSlot, (aggstate->top_k+aggstate->array_walker-1));
			(aggstate->top_k+aggstate->array_walker-1)->count = estimated_value;
		}
			...
			while(temp){
				if (temp->count < min){
					min = temp->count;
					min_index = aggstate->array_walker-1;
				}
				temp = approx_agg_advance_iter(aggstate, agg);
			}
			min=aggstate->top_k->count;

			if (estimated_value > (aggstate->top_k+min_index)->count){
				set_approx_top_entry_from_slot(outerSlot, (aggstate->top_k+min_index));
				(aggstate->top_k+min_index)->count = estimated_value;
			}
		}
	}
	approx_agg_reset_iter(aggstate);
*/
	...
	for (i=0;i<agg->approx_nkeep;i++){    //update if the tuple is already in the list
		if ((aggstate->top_k+i)->tuple==NULL)
			break;
		if (compare_tuple_with_approx_top_tuple(outerSlot, aggstate->top_k+i, aggstate, agg)){
			(aggstate->top_k+i)->count = estimated_value;
			found = true;
			break;
		}
	}
	...
	if (!found){
		...
		if (i<agg->approx_nkeep){  //array is not full
			set_approx_top_entry_from_slot(outerSlot, (aggstate->top_k+i));
			(aggstate->top_k+i)->count = estimated_value;
		}
	
		else{	//array is full
			min = (aggstate->top_k)->count;
			for (i=1;i<agg->approx_nkeep;i++){
				if ((aggstate->top_k+i)->count < min){
					min = (aggstate->top_k+i)->count;
					min_index = i;
				}
			}
			if (estimated_value > (aggstate->top_k+min_index)->count){
...
approx_agg_advance_iter(AggState *aggstate, Agg* agg)
{
	 ...
	 */
    /*
    ApproxTopEntry *return_value;
    if (aggstate->array_walker == agg->approx_nkeep){
	aggstate->array_walker = 0;
	return NULL;
    }
    return_value = aggstate->top_k + aggstate->array_walker;
    aggstate->array_walker++;
