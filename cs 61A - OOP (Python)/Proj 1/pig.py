...
Proj #1 Pig Game
Lab Section: 24
Zhaohua Gao  cs61a-kk
Pei Jun Chen cs61a-kh
'''
...
def roll(turn_total, outcome):
    ...
    if outcome == 1:
        ...
        point_in_turn += outcome
    return point_scored, point_in_turn, turn_is_over
...
def take_turn(plan, dice=make_fair_die(), who='Someone', comments=True):
    ...
    "*** YOUR CODE HERE ***"
    point_in_turn, turn_is_over = 0, False
    while turn_is_over == False:
        dice_outcome = dice()
        action = plan(point_in_turn)
        score_for_turn, point_in_turn, turn_is_over = action(point_in_turn, dice_outcome)
        if comments == True:
            commentate(action, dice_outcome, score_for_turn, point_in_turn, turn_is_over, who)
...
def averaged(fn, num_samples=200):
...
def run_strategy_experiments():
    ...
    "*** YOUR CODE HERE ***"
    #eval_strategy_range(make_roll_until_strategy,15,25)
    #eval_strategy_range(make_die_specific_strategy, 1, 10)
    #eval_strategy_range(make_pride_strategy, 0, 10)
    eval_strategy_range(test_final_strategy, 15, 35)
    ...
    def strategy(score, opponent_score):
        def plan(turn):
            ...
            if(score + opponent_score) % 7 == 0:
                if turn >= four_side_goal or (turn + score + opponent_score) % 7 ==0:
                    return hold
                else:
                    return roll
            else:
                if turn >= six_side_goal or (turn + score + opponent_score) % 7 ==0:
                    return hold
                else:
                    return roll
        return plan
...
def make_pride_strategy(margin, turn_goal=20):
    ...
    def strategy(score, opponent_score):
        def plan(turn):
            if (turn >= turn_goal) and (turn + score - opponent_score >= margin):
                return hold
            else:
                return roll
        return plan
...
def test_final_strategy(turn_goal):
    ...
    #"*** YOUR CODE HERE ***"
        def plan(turn):
            ...
            if 100 - score < turn_goal:
                new_goal = 100 - score
            if turn >= new_goal and (turn + score - opponent_score  >= 5) or (turn + score + opponent_score) % 7 == 0:
                return hold
            else:
                return roll
        return plan
...
def final_strategy(score, opponent_score):
    ...
    def plan(turn):
        ...
        if 100 - score < 24:
            new_goal = 100 - score
        if turn >= new_goal and (turn + score - opponent_score  >= 5) or (turn + score + opponent_score) % 7 == 0:                return hold
        else:
            return roll
    return plan               
