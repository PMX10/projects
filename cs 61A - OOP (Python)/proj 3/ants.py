...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if self.ant != None and self.ant.container:
                if self.ant.can_contain(insect):  #self.ant is container 
                    self.ant.contain_ant(insect)
            elif self.ant!= None and insect.container:
                if insect.can_contain(self.ant):  #the ant trying to add is container
                    new_ant = self.ant  #not ready
                    insect.contain_ant(new_ant)
                    self.ant = insect                    
            else:
...
class Ant(Insect):
    ...
    def can_contain(self, other_ant):
        if self.container:
            if self.ant == None and other_ant.container == False:
                return True
        return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        bee_found = False
        attack_range = 0
        while place.name != hive.name and bee_found == False:
            if  place.bees != [] and attack_range >= self.min_range:
                bee_found = True
            else: 
                attack_range += 1
                place = place.entrance
        if place.name != hive.name and attack_range >= self.min_range and attack_range <= self.max_range:
...
class HungryAnt(Ant):
    """HungryAnt will take three "turns" to eat a Bee in the same space or the
    ...
    def action(self, colony):
        ...
        if self.place.bees != []:
            if self.digesting == 0:
                self.eat_bee(random.choice(self.place.bees) if self.place.bees else None)
