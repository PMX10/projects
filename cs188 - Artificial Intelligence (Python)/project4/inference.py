...
class ExactInference(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #If ghost has been captured, weightage of jail position becomes 1 and other positions become 0
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    #Special case where ghost is captured
    if noisyDistance == None:
      allParticles[self.getJailPosition()] = 1
      #Since ghost is captured, there's only one particle left in the particle list
    ...
    for p in self.particles:
      ...
      #Use addition because particles may fall into the same position
      if emissionModel[distance] > 0: allParticles[p] += emissionModel[distance]

    #Check the weights of the particles
    allZero = True
    ...
    #Generate a new sample with the reweighted particles
    newSample = []
    allParticles.normalize()
    count = 0
    while count < self.numParticles:
      newSample.append(util.sample(allParticles))
      count += 1
      
    #The new sample set is reassigned to the game state
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    nextParticles = []
    #Get random samples from new ghost positions and use those as the new list of particles 
    for p in self.particles:
      newPostDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
      sample = util.sample(newPostDist)
      nextParticles.append(sample)
  ...
  def getBeliefDistribution(self):
    ...
    """
    "*** YOUR CODE HERE ***"
    #Create a new dictionary to contain the new ghost locations and their probabilities
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    self.particles = []
    p = 0
    #For every tuple in self.particles, it contains a tuple of all the ghosts' positions
    while p < numParticles:
      ghostLocation = []
      count = 0
      while count < numGhosts:
        ghostLocation.append(random.choice(legalPositions))
        count += 1
      p += 1
      self.particles.append(tuple(ghostLocation))
  ...
  def elapseTime(self, gameState):
    ...
    **Note** that you may need to replace "prevGhostPositions" with the
    ...
    for oldParticle in self.particles:
      ...
      count = 0
      tmp = []
      #For every position of the ghost's location, get the next position distribution and obtain a random sample for the next computation
      #When we obtained a new sample for every ghost, append the new tuple of ghost's location to the new empty list
      for location in newParticle:
        newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), count, self.ghostAgents[count])
  ...
  def observeState(self, gameState):
    ...
    ** Remember ** We store particles as tuples, but to edit a specific particle,
    ...
    #Check if any ghost is captured by pacman
    for particle in self.particles:
      ghostCount = 0
      tempVal = 1
      for nDist in noisyDistances:
        if nDist == None:
          par = self.getParticleWithGhostInJail(particle, ghostCount)
          #pars.append(par)
          #executed = True
        else:
          par = particle
          distance = util.manhattanDistance(pacmanPosition, particle[ghostCount])
          tempVal = tempVal * emissionModels[ghostCount][distance]
        ghostCount += 1
      allParticles[par] += tempVal
    ...
    for particle in self.particles:
      ghostCount2 = 0
      tempVal = 1
      #If not None, update the beliefs
      #Multiply the probability of 3 ghosts at their current location and add it to the dictionary
      #Use emissionModels to get the probability of observation
      for ghost in particle:
        distance = util.manhattanDistance(pacmanPosition, ghost)
        tempVal = tempVal * emissionModels[ghostCount2][distance]
        ghostCount2 += 1
      if particle not in pars:
        allParticles[particle] += tempVal
      else:
        continue
      """
    ...
    #Normalize and check if all weights are 0. If all weights are 0, re-initialize
    allParticles.normalize()
